...
Аудитория: technical
Статус: постановка до реализации
...
API хранит `Budget.date` как месяц. Правило, по которому месячные записи бюджета сопоставляются с периодом 20–19, публично не подтверждено. Пока это открытый контракт, а не готовая формула.
...
- границы `start_date` и `end_date` включительные;
- если `end_date` не передан, он вычисляется как текущая дата; ответ всегда содержит resolved `start_date` и `end_date`;
...
- Как месячный `Budget.date` сопоставляется с периодом 20–19 и диапазоном, который пересекает два месяца?
- Как период начинается 29, 30 или 31 числа в коротком месяце?
