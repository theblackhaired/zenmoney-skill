...
The advanced analytics handlers (`get_category_report`, `get_money_flow`, `get_income_outcome_comparison`, `get_balance_trend`) consume the refreshed cache after the normal prefetch. Category, comparison, and balance-series conversions use current `Instrument.rate` values received through the public `/v8/diff/` contract. Balance history is reconstructed from current account balances by reversing synced transactions after each requested point, and the response exposes that provenance in metadata.

Plans uses the same public-only conversion policy. A successful `/v8/diff/` is the authentication gate and the source of current `Instrument.rate` values; Plans records the policy in `metadata.currency_conversion`. The documented public API does not expose dated rate history, so historical multi-currency totals are current-rate estimates and may differ from the mobile application. A `401` from `/v8/diff/` remains `AUTHENTICATION_FAILED`.
