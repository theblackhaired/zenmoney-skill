"""Legacy budget-realm bootstrap; do not use for the current master-realm setup.

This script is retained for the earlier standalone Keycloak topology. All
credentials come from environment. Current deployment uses the existing master
realm and a separate, narrowly scoped configuration workflow.
...
Historical invocation: python deploy/bootstrap_realm.py (not for master).
...
AUDIENCE = "https://theblackhaired.ru/zenmoney/mcp"
