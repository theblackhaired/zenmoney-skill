...
from zenmoney import cache, transport
...
class TransportWriteConfirmationTests(unittest.TestCase):
    ...
    def _run_with_snapshot(self, changes, responses):
        cache.CACHE.apply_diff({'serverTimestamp': 0})

        async def fake_api_post(endpoint, body):
            self.assertEqual(endpoint, '/v8/diff/')
            return responses.pop(0)
        with patch.object(transport, '_api_post', AsyncMock(side_effect=fake_api_post)) as api_post:
            result = asyncio.run(transport._write_diff(changes))
            saved = {'serverTimestamp': cache.CACHE.server_timestamp, **{key: list(value.values()) for key, value in cache.CACHE.data.items()}}
            calls = [call.args for call in api_post.await_args_list]
        return (result, saved, calls)

    def test_write_is_confirmed_by_force_fetch_in_snapshot(self):
        tx = {'id': 'tx-1', 'changed': 10, 'deleted': False, 'outcome': 100}
        result, saved, calls = self._run_with_snapshot({'transaction': [tx]}, [{'serverTimestamp': 1, 'transaction': [tx]}, {'serverTimestamp': 2, 'transaction': [{**tx, 'outcome': 100}]}])
    ...
    def test_write_fails_when_server_confirmation_lacks_written_entity(self):
        ...
        cache.CACHE.apply_diff({'serverTimestamp': 0})
        with patch.object(transport, '_api_post', AsyncMock(side_effect=[{'serverTimestamp': 1}, {'serverTimestamp': 2, 'transaction': []}])):
    ...
    def test_write_fails_when_server_keeps_stale_entity_values(self):
        ...
        cache.CACHE.apply_diff({'serverTimestamp': 0})
        with patch.object(transport, '_api_post', AsyncMock(side_effect=[{'serverTimestamp': 1}, {'serverTimestamp': 2, 'transaction': [{**tx, 'outcome': 50}]}])):
    ...
    def test_write_allows_server_normalization_of_derived_transaction_fields(self):
        tx = {'id': 'tx-normalized', 'changed': 10, 'deleted': False, 'outcome': 100, 'payee': 'Store', 'originalPayee': None, 'opIncome': None, 'opOutcome': None}
        confirmed = {**tx, 'originalPayee': 'Store', 'opIncome': 0, 'opOutcome': 0}
        result, saved, _ = self._run_with_snapshot({'transaction': [tx]}, [{'serverTimestamp': 1, 'transaction': [tx]}, {'serverTimestamp': 2, 'transaction': [confirmed]}])
        self.assertEqual(result['serverTimestamp'], 1)
    ...
    def test_budget_write_confirms_synthetic_key(self):
        budget = {'user': 1, 'changed': 10, 'tag': None, 'date': '2026-07-01', 'income': 0, 'outcome': 5000}
        _, saved, calls = self._run_with_snapshot({'budget': [budget]}, [{'serverTimestamp': 1, 'budget': [budget]}, {'serverTimestamp': 2, 'budget': [budget]}])
    ...
    def test_deletion_requires_server_absence_and_removes_from_cache(self):
        ...
        cache.CACHE.apply_diff({'serverTimestamp': 0, 'reminder': [existing]})
        with patch.object(transport, '_api_post', AsyncMock(side_effect=[{'serverTimestamp': 1}, {'serverTimestamp': 2, 'reminder': []}])):
